...
function processFile(f, fname, inputdir, out) {
  ...
  out.write('<HTML><HEADER><TITLE>' + fname + '</TITLE><BODY>');
  if (inputdir != null) {
  ...
  while ((s = f.readLine()) != null) {
    ...
    if (m != null) {
      ...
      do {
        ...
        comment += (new String(s)).replace(/^\s*\*/, "");
        s = f.readLine();
      } while (s != null);  
      ...
      if (firstLine) {
        ...
        out.write('<H1>File ' + fname + '</H1>');
        out.write(processComment(comment,firstLine,fname));
        out.write('<HR>');
        firstLine = false;
        comment = "";
        continue;
  ...
  for (var i=0; i < functionDocArray.length; i++) {
    with (functionDocArray[i]) {
      out.write('function <A HREF=#' + name +
              '>' + name + '</A><BR>');
    }
  }
  out.write('<HR>');
...
function CreateOutputFile(outputdir,htmlfile)  {    
  if (outputdir==null)  {
    ...
    var outname = outputdir + File.separator + (new File(htmlfile)).getName();
  }
  print("output file: " + outname);

  return new FileWriter(outname);
}
...
function processJSFile(filename,inputdir)  {
  if (debug)  {
    print("filename = " + filename + " inputdir = " + inputdir);
  }
  ...
  if (!filename.match(/\.js$/)) {
    ...
    var f = new LineNumberReader(new FileReader(inname));
...
function GenerateIndex(dirname)  {
  ...
  out.write('<HTML><HEADER><TITLE>File Index - directory: ' + dirname + '</TITLE><BODY>');
  out.write('<H1>File Index - directory: ' + dirname + '</H1>\n');
  out.write('<TABLE WIDTH="90%" BORDER=1>');
  out.write('<TR BGCOLOR=0xdddddddd>');
  out.write('<TD><B>File</B></TD>');
  out.write('<TD><B>Description</B></TD></TR>');
  ...
  for (var i=0; i < sortedFileArray.length; i++) {
    ...
    out.write('<TR><TD><A HREF=\"' + htmlfile + '\">' + fname + '</A></TD></TD><TD>');
    if (indexFileArray[fname])  {
      out.write(indexFileArray[fname]);
    } else  {
  ...
  // write the beginning of the file
  out.write('<HTML><HEADER><TITLE>Function Index - directory: ' + dirname + '</TITLE><BODY>');
  out.write('<H1>Function Index - directory: ' + dirname + '</H1>\n');
  out.write('<TABLE WIDTH="90%" BORDER=1>');
  out.write("<tr bgcolor=0xdddddddd>");
  out.write("<td><b>Function</b></td>");
  out.write("<td><b>Files</b></td></tr>");
...
for (var j in DirList) {
  ...
  var inputDirName = inputdir.replace(/\.\.\/|\.\.\\/g,"");
inputDirName="";
//print("inputdir="+inputdir);
//print("inputDirName="+inputDirName);
