    ...
    extends AbstractProgramImporter
{
     ...
     *
     * @param _instance         instance (object id) of the ESJP
    ...
    {
        try {
            final InputStream is = new ByteArrayInputStream(getCode().toString().getBytes(ESJPImporter.ENCODING));
            final Checkin checkin = new Checkin(_instance);
            checkin.executeWithoutAccessCheck(getProgramName(), is, getCode().length());
        } catch (final UnsupportedEncodingException e) {
            throw new EFapsException(getClass(), "updateDB.UnsupportedEncodingException", e);
        }
    }
        ...
        throws EFapsException
    {
        ...
        final Type esjpType = Type.get(EFapsClassNames.ADMIN_PROGRAM_JAVA);
