...
public class EFapsInstance implements Scriptable {
   ...
   * System.out.println("ein-constructor"); try { } catch (Exception e) {
   * e.printStackTrace(); } }
   */
  ...
  public void jsConstructor(String _type, String _name) {
    ...
    try {
      ...
      if (nameAttr == null) {
        instance = new Instance(type, 0);
      } else {
        SearchQuery query = new SearchQuery();
        query.setQueryTypes(_type);
        query.addWhereExprEqValue("Name", _name);
        query.addSelect("Name");
        query.execute();
        if (query.next()) {
          instance = new Instance(query.getRowOIDs(Shell.context));
        } else {
          instance = new Instance(type, 0);
        }
      }
   ...
   * the form of <code>get</code> that takes an integer: <table>
   * <tr>
    ...
    try {
      ...
      if (_name.equals("oid")) {
        ...
        if (ret == null) {
          ...
          if (attr != null) {
            SearchQuery query = new SearchQuery();
            query.setObject(self.instance);

            if (AbstractLinkType.class.isAssignableFrom(attr.getAttributeType()
                .getClassRepr())) {
              query.addSelect(_name + ".OID");
            }
            query.addSelect(_name);
            ...
            if (query.next()) {
              ...
              if (result instanceof Type) {
                ...
                /*
                 * } else if (result instanceof AbstractLinkType) { result =
                 * query.get(Shell.getContext(), _name+".OID"); String oid =
                 * result.getViewableString(Shell.getContext().getLocale()); ret =
                 * new EFapsInstance(); ((EFapsInstance)ret).instance = new
                 * Instance(Shell.context, oid);
                 */
              } else {
                result = ret;
              }
            ...
            while (query.next()) {
              list.add(new EFapsInstance(new Instance(query.getRowOIDs(Shell
                  .getContext()))));
            }
            query.close();
   ...
   * as the <i>start</i> parameter. To preserve JavaScript semantics, it is the
  ...
  public void put(String _name, Scriptable _object, Object _value) {
    ...
    if (_value instanceof Function) {
      ...
      if (attr != null) {
        ...
        System.err.println("Attribute '" + _name
            + "' does not exists for type '"
            + self.instance.getType().getName() + "'.");
      }
    }
  }
   ...
   * [[Delete]] except that the no result is returned. The runtime will
  ...
  public void jsFunction_attributes() {
    System.out.println("Type '" + instance.getType().getName()
        + "' has following attributes:");
    for (Iterator iter = instance.getType().getAttributes().entrySet()
