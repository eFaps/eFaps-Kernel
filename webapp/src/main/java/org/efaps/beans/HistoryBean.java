...
/**
 * The bean is used to select the history data from the database and return this
 * to the JSP page.
 * 
 * @author tmo
 * @version $Rev$
...
public class HistoryBean extends AbstractBean {
  ...
  /**
   * The string stores the name of the column with was used to sort. The value
   * is updated after a new sort is done in {@link #doSort}.
   * 
   * @see #doSort
   */
  ...
  /**
   * The value is <i>true</i> if the list was sorted ascending. The value is
   * updated after a new sort is done in {@link #doSort}.
   * 
   * @see #doSort
   */
   ...
   * history data is read with {@link #doExecute} and sorted {@link #doSort} by
   * the modified column descending.
   * 
   * @param _oid
  ...
  /**
   * All history data depending on the object id is read and stored in instance
   * variable {@link #data}.
   * 
   * @throws EFapsException
   *           if no instance is given, from called methods, or if another
   *           exception is thrown
   */
  protected void doExecute() throws EFapsException {
    ...
    try {
      ...
      try {
        ...
        stmt.execute("select EVENTTYPEID,MODIFIED,MODIFIER,ATTRID,ATTRVALUE "
            + "from HISTORY " + "where FORTYPEID=" + instance.getType().getId()
            + " " + "and FORID=" + instance.getId());

        List<List<ColumnValue<?>>> rows = new ArrayList<List<ColumnValue<?>>>();

        ResultSet rs = stmt.getResultSet();
        while (rs.next()) {
          List<ColumnValue<?>> row = new ArrayList<ColumnValue<?>>();
          String eventType = "History.Event."
              + Type.get(rs.getLong(1)).getName();
  ...
  protected void doSort() {
    if ((this.sortColumn != null)
        && ((this.sortAscending != this.sortedAscending) || !this.sortColumn
            .equals(this.sortedColumn))) {
      ...
      Comparator<List<ColumnValue>> comparator = new Comparator<List<ColumnValue>>() {
        public int compare(List<ColumnValue> _o1, List<ColumnValue> _o2) {
          ...
          return sortAscending ? ((Comparable) val1.getValue()).compareTo(val2
              .getValue()) : ((Comparable) val2.getValue()).compareTo(val1
              .getValue());
        }
      };

      Collections.sort((List<List<ColumnValue>>) this.data.getWrappedData(),
          comparator);
  ...
  public Object getColumnValue() {
    ...
    if (this.data.isRowAvailable() && this.columnHeaders.isRowAvailable()) {
      columnValue = ((List) data.getRowData()).get(this.columnHeaders
          .getRowIndex());
    }
    return columnValue;
  }
