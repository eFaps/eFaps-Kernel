...
public abstract class AbstractFilter implements Filter  {
   ...
   */
  private final static Log LOG = LogFactory.getLog(AbstractFilter.class);
   ...
   * be overwritten by derived classes.<br/>
   ...
   * within the filter's doFilter method have exited or after a timeout period
   * has passed. After the web container calls this method, it will not call
   * the doFilter method again on this instance of the filter.<br/>
  ...
  public void doFilter(final ServletRequest _request, 
    ...
    if ((_request instanceof HttpServletRequest) 
      ...
      doFilter((HttpServletRequest) _request, 
  ...
  protected void setLoggedInUser(final HttpServletRequest _request,
    ...
    if (_userName == null)  {
      _request.getSession(true)
              .removeAttribute(SESSIONPARAM_LOGIN_NAME);
    } else  {
      _request.getSession(true)
  ...
  protected String getLoggedInUser(final HttpServletRequest _request)  {
    return (String) _request.getSession(true)
                            .getAttribute(SESSIONPARAM_LOGIN_NAME);
  }
}
