...
public class MenuComponent extends WebComponent {
  ...
  public MenuComponent(final String _id, 
                       final IModel _model,
...
System.out.println("getMarkupId()="+getMarkupId());
System.out.println("_openTag.id="+_openTag.getId());
System.out.println("_openTag="+_openTag);
System.out.println("_openTag="+_openTag.getString("id"));

      super.replaceComponentTagBody(_markupStream, _openTag, convertToHtml(_openTag));
    } catch (Exception e) {
...
//    appendCSS(_openTag, html);
    ...
    html.append(JavascriptUtils.SCRIPT_OPEN_TAG)
        .append("var ").append(id).append("=[");
...
// Todo: id darf nicht hard-coded sein!!!
    ...
    html.append("];")
        .append("cmDraw ('").append(id).append("', ").append(id).append(", 'hbr', cmThemeOffice);")
  ...
  protected void appendCSS(final ComponentTag _openTag,
    ...
    _html.append(CssUtils.INLINE_OPEN_TAG)
  ...
  public void convertToHtml(final IMenuItemModel _menuItem,
      ...
      for (IMenuItemModel menuItem : _menuItem.childs)  {
        ...
        convertToHtml(menuItem, _html, false, new StringBuilder(_prefix).append("  "));
      }
      _html.append("]");
  }
}
