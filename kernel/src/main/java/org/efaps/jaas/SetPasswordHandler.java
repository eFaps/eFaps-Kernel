...
public class SetPasswordHandler {
  ...
  /**
   * Constructor to initialize the setpassword handler. If <i>null</i> is given
   * to the application name, the default value defined in {@link #application}
   * is used.
   *
   * @param _application
  ...
  public boolean setPassword(final String _name, final String _newpasswd,
                             final String _oldpasswd) throws LoginException {
    boolean ret = false;

    final LoginContext login =
        new LoginContext(this.application, new SetPasswordCallbackHandler(
            ActionCallback.Mode.SET_PASSWORD, _name, _newpasswd, _oldpasswd));
    login.login();
    ret = true;
