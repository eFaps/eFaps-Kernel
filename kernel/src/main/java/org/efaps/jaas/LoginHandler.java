...
public class LoginHandler  {
   ...
   *         context
   * @throws EFapsException if more than one person for given JAAS login context
   *         is found or a method of the principals inside the JAAS login 
...
// TODO: throw exception!!
  ...
  /**
   * The person information inside eFaps is update with information from 
   * JAAS login context.
   *
   * @param _login  JAAS login context
   * @param _person Java person instance inside eFaps to update
   * @throws EFapsException if a method of the principals inside the JAAS login 
   *         contexts could not be executed or the person could not be updated
   *         from the values in the JAAS login context.
   */
  protected void updatePerson(final LoginContext _login,
    ...
    for (JAASSystem system : JAASSystem.getAllJAASSystems())  {
      ...
      for (Object persObj : users)  {
        try  {
          for (Map.Entry < Person.AttrName, Method > entry
                            : system.getPersonMethodAttributes().entrySet())  {
            _person.updateAttrValue(
                entry.getKey(),
...
// TODO: throw exception!!
   ...
   * @throws EFapsException if a method of the principals inside the JAAS login 
   *         contexts could not be executed or the roles for the given person
   *         could not be set.
   */
  protected void updateRoles(final LoginContext _login, 
    ...
    for (JAASSystem system : JAASSystem.getAllJAASSystems())  {
      if (system.getRoleJAASPrincipleClass() != null)  {
        Set rolesJaas = _login.getSubject()
                              .getPrincipals(system.getRoleJAASPrincipleClass());
