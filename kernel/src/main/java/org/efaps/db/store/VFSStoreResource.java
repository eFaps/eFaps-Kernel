 ...
 *
 * @author tmo
 * @version $Id: VFSStoreResource.java 2142 2009-01-28 21:51:50Z jmox $
 */
public class VFSStoreResource extends AbstractStoreResource {
   ...
   */
  public static final String EXTENSION_TEMP = ".tmp";

  /**
   * Extension of a file in the store.
   */
  public static final String EXTENSION_NORMAL = "";
   ...
   */
  public static final String PROPERTY_NUMBER_SUBDIRS
                                                = "VFSNumberSubDirectories";

  /**
   * Property Name to define if the type if is used to define a sub directory.
   */
  public static final String PROPERTY_USE_TYPE = "VFSUseTypeIdInPath";

  /**
   * Property Name to define if the type if is used to define a sub directory.
   */
  public static final String PROPERTY_NUMBER_BACKUP = "VFSNumberBackups";

  /**
   * Property Name to define the base name.
   */
  public static final String PROPERTY_BASENAME = "VFSBaseName";

  /**
   * Property Name for the class name of the Provider.
   */
  public static final String PROPERTY_PROVIDER = "VFSProvider";

  /**
   * Logging instance used in this class.
   */
  private static final Logger LOG
                              = LoggerFactory.getLogger(VFSStoreResource.class);
  ...
  public void initialize(final Instance _instance,
    ...
    final String useTypeIdStr = _properties.get(PROPERTY_USE_TYPE);
    if ("true".equalsIgnoreCase(useTypeIdStr))  {
      fileNameTmp.append(getType().getId()).append("/");
    }

    final String numberSubDirsStr =  _properties.get(PROPERTY_NUMBER_SUBDIRS);
    ...
    final String numberBackupStr = _properties.get(PROPERTY_NUMBER_BACKUP);
  ...
  public DefaultFileSystemManager evaluateFileSystemManager()
    ...
    final String baseName =  getProperties().get(PROPERTY_BASENAME);
    final String provider =  getProperties().get(PROPERTY_PROVIDER);
    try {
      ret.init();
  ...
  public int write(final InputStream _in, final int _size)
    ...
    try  {
      ...
      int size = _size;
      final FileObject tmpFile = this.manager.resolveFile(this.manager
          .getBaseFile(), this.fileName + EXTENSION_TEMP);
      if (!tmpFile.exists()) {
        tmpFile.createFile();
      }
  ...
  public InputStream read() throws EFapsException  {
    ...
    try  {
      ...
      final FileObject file = this.manager.resolveFile(this.fileName
          + EXTENSION_NORMAL);
   ...
   */
  private void backup(final FileObject _backup, final int _number)
      throws FileSystemException {
    if (_number < this.numberBackup) {
      final FileObject backFile = this.manager.resolveFile(this.manager
          .getBaseFile(), this.fileName + EXTENSION_BACKUP + _number);
      if (backFile.exists()) {
  ...
  public void commit(final Xid _xid, final boolean _onePhase)
    ...
    if (this.storeEvent == StoreEvent.WRITE) {
      try {
        final FileObject tmpFile = this.manager.resolveFile(this.manager
            .getBaseFile(), this.fileName + EXTENSION_TEMP);
        final FileObject currentFile = this.manager.resolveFile(this.manager
            .getBaseFile(), this.fileName + EXTENSION_NORMAL);
        final FileObject bakFile = this.manager.resolveFile(this.manager
            .getBaseFile(), this.fileName + EXTENSION_BACKUP);
      ...
      try {
        final FileObject curFile = this.manager.resolveFile(this.manager
            .getBaseFile(), this.fileName + EXTENSION_NORMAL);
        final FileObject bakFile = this.manager.resolveFile(this.manager
            .getBaseFile(), this.fileName + EXTENSION_BACKUP);
        if (bakFile.exists()) {
