...
/**
 * @author tmo
 * @version $Id: LifeCycleAction.java 466 2006-10-20 15:15:42 +0000 (Fri, 20 Oct
 *          2006) tmo $
 * @todo description
 */
public class LifeCycleAction extends Update {
  ...
  public void execute(final int _action) throws Exception {
    ...
    try {
      ...
      add(getStatusAttribute(), "" + status.getId());
      super.execute();
   ...
   * 
   * @param _curStatus
   *          status for which to get next status
   */
   ...
   * 
   * @param _curStatus
   *          status for which to get previous status
   */
   ...
   * @param _update
   *          <i>true</i> means, the selction is with <b>for update</b>,
   *          <i>false</i> means, the seleection is without <b>for update</b>.
   * @return current status
   * @see #getStatusAttribute
   */
  protected Status getCurrentStatus(Context _context, boolean _update)
                                                                      throws Exception {
    ...
    StatusLinkType statusValue = (StatusLinkType) query.get(statusAttr);
