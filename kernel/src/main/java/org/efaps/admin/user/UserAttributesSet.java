 ...
 * allready in the eFpas-DataBase existing UserAttribute and store them in a
 * map. A new or altered UserAttribute wich is created/altered during the
 * Session will only be stored in the map and not into the eFaps-DataBase. To
 * store the UserAttribute into the eFapsDataBase the mehto {@link #storeInDb()}
...
public class UserAttributesSet {
  ...
  /**
   * constructor using the constructor {@link #UserAttributesSet(long)} through
   * searching the Id for the given Name
   *
   * @param _userName
   *                UserName of the User this UserAttributesSet will belong to
   ...
   *
   * @param _userId
   *                Id of the User this UserAttributesSet will belong to
   * @throws EFapsException
   */
  ...
  /**
   * method to check if this UserAttributesSet contains a UserAttribute with the
   * given key
   *
   * @param _key
   *                key to check if this UserAttributesSet contains it
   ...
   *
   * @param _key
   *                Key for the Value, that should be returned as String
   ...
   * to set the the Relation. It will search in the {@link #MAPPER} to retrieve
   * the UserAttributesDefinition. If found it will use the found one, else it
   * will use the a default (UserAttributesDefinition.ATTRIBUTE).
   *
   * @param _key
   *                key to be set
   * @param _value
  ...
  public void set(final String _key, final String _value,
    ...
    if (_key == null || _value == null) {
      throw new EFapsException(this.getClass(), "set", _key, _value,
          _definition);
    } else {
  ...
  /**
   * this method reads all UserAttribute from the eFaps-DataBase which belong to
   * the User this UserAttributesSet is realted to and stores them in a Map for
   * fast access.
   *
   * @see #UserAttribute
   * @throws EFapsException
