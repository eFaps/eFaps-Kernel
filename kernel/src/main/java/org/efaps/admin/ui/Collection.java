...
if (_field.getReference()!=null && _field.getReference().length()>0)  {
   ...
   * If the field expression is already added, the old index is returned, so
   * a expression is only added once.
   *
...
this.selIndexLen++;
   ...
   * variables are also cloned:<br/>
   * - The array of fields {@link #fields} is also cloned, but not the fields itself!<br/>
   * - {@link #allFieldExpr}
   * - {@link #expandAttributes}
   */
/*  protected Object clone() throws CloneNotSupportedException  {
    Collection collection = (Collection)super.clone();
    collection.fields = getFields().clone();
    collection.allFieldExpr = (Hashtable)getAllFieldExpr().clone();
...
/*    protected void setProperty(String _name, String _value) throws EFapsException  {
  if (_name.startsWith("ExpandAttribute"))  {
      ...
      if (getExpandAttributes().size()<index)  {
        getExpandAttributes().setSize(index);
      }
      getExpandAttributes().set(index-1, Attribute.get(_context, _value));
    } else if (_name.equals("FooterMenu"))  {
      setFooterMenu(Menu.get(_context, _value));
 ...
 *
 * @param _select new value for the {@link #hRefBottom}
 * @see #hRefBottom
 * @see #getHRefBottom
 */
