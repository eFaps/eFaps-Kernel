...
public class MenuChilds implements EventExecution {
   ...
   * table can be sorted in the same order as they are rendered from the webapp.
   * It is called as an UI_FIELD_VALUE event from within table
  ...
  public Return execute(final Parameter _parameter) throws EFapsException {
    ...
    final Instance callinstance = (Instance) _parameter
  ...
  /**
   * Method is called from a command to move the child of a menu on position up.
   *
   * @param _parameter  Parameter as provided to the esjp from eFaps
   ...
   * @throws EFapsException on error
   */
  private void move(final Parameter _parameter, final boolean _up)
      throws EFapsException {
    ...
    if (oidRows != null) {
      ...
      for (final String oidRow : oidRows) {
        ...
        if (query.next()) {
          ...
          final Map<String, String> id2cmd = new HashMap<String, String>();
          while (query2.next()) {
            final String id = query2.get("ID").toString();
            final String cmdId = query2.get("ToCommand").toString();
            ids.add(id);
            id2cmd.put(id, cmdId);
          }

          for (int i = 0; i < ids.size(); i++) {
            final String actId = ids.get(i);
            final String actCmd = id2cmd.get(actId);

            if (selCmdId.equals(actCmd)) {
              String exId = null;
              ...
              if (exId != null) {
                // update the exChange
                final Update update = new Update("Admin_UI_Menu2Command", exId);
                update.add("ToCommand", actCmd);
                update.execute();
                update.close();

                // update the actual
                final Update updateAct = new Update("Admin_UI_Menu2Command",
                    actId);
