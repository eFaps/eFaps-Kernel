...
abstract class AbstractMojo implements Mojo  {
  ...
  /**
   * <code>null</code> is returned, of the version file could not be opened and
   * read.
   *
...
fileSet.addIncludeFile(".*xml$");
      ...
      getLog().error("Could not open / read version file "
                     + "'" + this.versionFile + "'");
    } catch (Exception e)  {
    }
    return appl;
  ...
  protected boolean initDatabase()  {
    ...
    String dbClass   = null;
    try  {
      AbstractDatabase dbType = ((Class < AbstractDatabase > )Class.forName(this.type)).newInstance();
      if (dbType == null)  {
      ...
      getLog().error("could not found database description class "
                + "'" + dbClass + "'", e);
    } catch (InstantiationException e)  {
      getLog().error("could not initialise database description class "
                + "'" + dbClass + "'", e);
    } catch (IllegalAccessException e)  {
      getLog().error("could not access database description class "
                + "'" + dbClass + "'", e);
    }

    // buildup reference and initialise datasource object
    Reference ref = new Reference(DataSource.class.getName(), this.factory, null);
    ...
    try  {
      ...
      getLog().error("could not found data source class "
                     + "'" + ref.getFactoryClassName() + "'", e);
    } catch (InstantiationException e)  {
      getLog().error("could not initialise data source class "
                     + "'" + ref.getFactoryClassName() + "'", e);
    } catch (IllegalAccessException e)  {
      getLog().error("could not access data source class "
                     + "'" + ref.getFactoryClassName() + "'", e);
    }
    if (of != null)  {
      DataSource ds = null;
      try  {
        ...
        getLog().error("coud not get object instance of factory "
                       + "'" + ref.getFactoryClassName() + "'", e);
      }
      if (ds != null)  {
